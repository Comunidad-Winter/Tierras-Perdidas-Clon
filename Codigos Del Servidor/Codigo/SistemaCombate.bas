...
Select Case UCase$(Clase)
    Case "GUERRERO"
        ModificadorEvasion = otEvaciones.Guerrero
    Case "CAZADOR"
        ModificadorEvasion = otEvaciones.Cazador
    Case "PALADIN"
        ModificadorEvasion = otEvaciones.Paladin
    Case "BANDIDO"
        ModificadorEvasion = 0.9
    Case "ASESINO"
        ModificadorEvasion = otEvaciones.Asesino
    Case "PIRATA"
        ModificadorEvasion = 0.9
    Case "LADRON"
        ModificadorEvasion = otEvaciones.Ladron
    Case "BARDO"
        ModificadorEvasion = otEvaciones.Bardo
    Case Else
        ModificadorEvasion = otEvaciones.Others
End Select
End Function
...
Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbRed & "°-" & daño & "°" & str(Npclist(NpcIndex).Char.CharIndex))
'[KEVIN]
...
Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbRed & "°-" & daño & "°" & str(UserList(UserIndex).Char.CharIndex))
...
Else
Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbRed & "°" & "Falla" & "°" & UserList(UserIndex).Char.CharIndex)
...
Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbRed & "°" & "¡Fallas!" & "°" & Npclist(NpcIndex).Char.CharIndex)
End If
...
Call SendData(SendTarget.ToPCArea, VictimaIndex, UserList(VictimaIndex).Pos.Map, "||" & vbRed & "°-" & daño & "°" & str(UserList(VictimaIndex).Char.CharIndex))
...
If ExpaDar > 0 Then
    ...
    Else
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Has ganado " & ExpaDar & " puntos de experiencia." & FONTTYPE_DAREXP)
    End If

...
Dim TempY As Integer
    ...
    H = UserList(UserIndex).Stats.UserHechizos(UserList(UserIndex).flags.Hechizo)
         'Big Bang Hame Kame Ha - Hechizo mata a todos. Dylan.- [2011]
           If Hechizos(H).RompeTodo = 1 Then
            b = True
            For TempX = PosCasteadaX - 12 To PosCasteadaX + 12
                For TempY = PosCasteadaY - 12 To PosCasteadaY + 12
                    If InMapBounds(TempX, TempY) Then
                        If MapData(PosCasteadaM, TempX, TempY).UserIndex > 0 Then
                        If Not EsGM(MapData(PosCasteadaM, TempX, TempY).UserIndex) Then
                         Call UserDie(MapData(PosCasteadaM, TempX, TempY).UserIndex)
            ...
            For TempX = PosCasteadaX - 8 To PosCasteadaX + 8
                For TempY = PosCasteadaY - 8 To PosCasteadaY + 8
                    If InMapBounds(TempX, TempY) Then
                        If MapData(PosCasteadaM, TempX, TempY).UserIndex > 0 Then
                        If Not EsGM(MapData(PosCasteadaM, TempX, TempY).UserIndex) Then
                         If UserList(MapData(PosCasteadaM, TempX, TempY).UserIndex).flags.Muerto = 1 Then
                         Call RevivirUsuario(MapData(PosCasteadaM, TempX, TempY).UserIndex)
                        End If
                         End If
                        End If
                    End If
...
ElseIf Hechizos(hIndex).SubeHP = 2 Then
    ...
    End If
If UserList(UserIndex).Invent.CascoEqpObjIndex > 0 Then
        daño = (daño - ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).ResistenciaMagica)
    End If
...
'End If
    ...
    Call SendData(SendTarget.ToPCArea, tempChr, UserList(tempChr).Pos.Map, "||" & vbRed & "°-" & daño & "°" & str(UserList(tempChr).Char.CharIndex))

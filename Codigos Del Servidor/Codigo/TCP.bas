...
UserList(UserIndex).Stats.Honor = 0
...
Select Case UCase$(UserRaza)
    ...
    Case "ELFO"
        ...
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Constitucion) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Constitucion)
    Case "ELFO OSCURO"
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Fuerza) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Fuerza) + 2
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad)
        ...
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Carisma) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Carisma)
...
Dim Y As Integer
    'encriptar
   sndData = ModDesEncrypt.Encriptar(sndData)
    'encriptar
...
If NumUsers > recordusuarios Then
    Call SendData(SendTarget.ToAll, 0, 0, "||Record de usuarios conectados simultaneamente." & "Hay " & NumUsers & " usuarios." & FONTTYPE_INFO)
...
Dim TieneAdvs As String
TieneAdvs = GetVar(CharPath & UserList(UserIndex).name & ".CHR", "PENAS", "Cant")
If val(TieneAdvs) <> 0 Then
SendData SendTarget.ToIndex, UserIndex, 0, "||Tenes " & val(TieneAdvs) & " de 5 advertencias, para ver los motivos, escribí /ADVERTENCIAS." & "~255~30~30~1~0"
End If
...
Call SendData(SendTarget.ToIndex, UserIndex, UserList(UserIndex).Pos.Map, "PZ" & UserList(UserIndex).Stats.UserAtributos(Fuerza))
Call SendData(SendTarget.ToIndex, UserIndex, UserList(UserIndex).Pos.Map, "PM" & UserList(UserIndex).Stats.UserAtributos(Agilidad))
Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Los mensajes privados están " & Tienemsj & "." & FONTTYPE_VERDEN)
Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Ganá puntos de torneo, objetos exclusivos y muchos premios más ayudándonos a mantener el Servidor Online con Donaciones. Vía SMS: Envía REM LHIRIUS al 27777. Vía Boleta: Escribe /DONAR" & FONTTYPE_BLANCON)

If UserList(UserIndex).Stats.DonoBoleta > 0 Then
Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Tienes un premio de donación para recibir. Escribe /MIPREMIO." & FONTTYPE_BLANCON)
End If
If UserList(UserIndex).Stats.DonoSms > 0 Then
Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Tienes un premio de donación para recibir. Escribe /MIPREMIOSMS." & FONTTYPE_BLANCON)
End If
                ...
                rData = Right$(rData, Len(rData) - 6)
                    ...
                    Call ConnectNewUser(UserIndex, ReadField(1, rData, 44), ReadField(2, rData, 44), ReadField(4, rData, 44), ReadField(5, rData, 44), ReadField(6, rData, 44), _
                                        ...
                                        ReadField(14, rData, 44), ReadField(15, rData, 44), ReadField(16, rData, 44), ReadField(17, rData, 44), ReadField(18, rData, 44), ReadField(19, rData, 44), ReadField(20, rData, 44), _
                                         ReadField(21, rData, 44), ReadField(22, rData, 44), ReadField(23, rData, 44), ReadField(24, rData, 44), ReadField(25, rData, 44), ReadField(26, rData, 44), ReadField(27, rData, 44), ReadField(28, rData, 44))
...
SendData SendTarget.ToIndex, UserIndex, 0, "||No hay ningun torneo, por ende no se pueden entrgar los premios." & FONTTYPE_SERVER
Exit Sub
End If
    rData = Right$(rData, Len(rData) - 8)
    tIndex = NameIndex(rData)
Dim trofeosbronc As Obj
trofeosbronc.ObjIndex = 877
trofeosbronc.Amount = 1
 If Not tIndex > 0 Then Exit Sub
    If Not MeterItemEnInventario(tIndex, trofeosbronc) Then
            Call TirarItemAlPiso(UserList(tIndex).Pos, trofeosbronc)
    End If
Call SendData(SendTarget.ToAll, UserIndex, 0, "||" & UserList(UserIndex).name & " le entrega 1 trofeo de bronce a " & UserList(tIndex).name & " por haber salido tercero en el torneo." & "~187~136~0~1~0")
   Dim Cantidad As String
   If Torne.Cupos = "16" Then
        Cantidad = 6
    Else
        Cantidad = val(Torne.Cupos / 4)
    End If
Exit Sub
End If
...
trofeosoro.Amount = 1
trofeosoro.ObjIndex = 879
 If Not tIndex > 0 Then Exit Sub
    If Not MeterItemEnInventario(tIndex, trofeosoro) Then
            Call TirarItemAlPiso(UserList(tIndex).Pos, trofeosoro)
    End If
    Call SendData(SendTarget.ToAll, UserIndex, 0, "||" & UserList(UserIndex).name & " le entrega 1 trofeo de oro a " & UserList(tIndex).name & " por haber salido primero en el torneo." & "~233~198~1~1~0")
Exit Sub
End If
 
 
If UCase$(Left$(rData, 9)) = "/SUMUSERS" Then
If Torne.Existe = False Then
SendData SendTarget.ToIndex, UserIndex, 0, "||No hay ningun torneo, por ende no se pueden sumonear los m_Users." & FONTTYPE_SERVER
Exit Sub
End If
Dim MaxX As Byte
Dim MinX As Byte
Dim W As Integer
Select Case Torne.Tipo
     
     Case Is = "DE DEATHMACHT"
            MinX = 69 ' coordenada x..
            MaxX = 79 ' coordenada y..
        For i = 1 To Torneo.Longitud
                Call WarpUserChar(i, 49, 77, 55, True) ' cambiar 49 por su mapa de torneo <<
        Next i
End Select
...
If UCase$(Left$(rData, 4)) = "/CR " Then
    rData = Right$(rData, Len(rData) - 4)
    rData = val(rData)
    If rData <= 0 Or rData >= 200 Then Exit Sub
...
If UCase$(Left$(rData, 13)) = "/ADVERTENCIA " Then
    ...
    On Error GoTo Err
        tInt = val(GetVar(CharPath & name & ".chr", "PENAS", "Cant"))
        Call WriteVar(CharPath & name & ".chr", "PENAS", "Cant", tInt + 1)
        If GetVar(CharPath & name & ".chr", "PENAS", "Cant") = 5 Then
        Call WriteVar(CharPath & name & ".chr", "PENAS", "P" & tInt + 1, ": Acumulación de advertencias - Pena aplicada por: " & UserList(UserIndex).name & " - Fecha: " & Date & " " & Time)
        Else
        Call WriteVar(CharPath & name & ".chr", "PENAS", "P" & tInt + 1, ": Advertencia por: " & tStr & " - Pena aplicada por: " & UserList(UserIndex).name & " - Fecha: " & Date & " " & Time)
        End If
        Call SendData(SendTarget.ToAll, UserIndex, 0, "||" & UserList(UserIndex).name & " advirtió a " & LCase(name) & "." & FONTTYPE_SERVER)
        If GetVar(CharPath & name & ".chr", "PENAS", "Cant") = 1 Then
        i = 5
        Call SendData(SendTarget.ToIndex, tIndex, 0, "||Tenes " & i & " minutos de carcel para reflexionar." & FONTTYPE_TALK)
        ElseIf GetVar(CharPath & name & ".chr", "PENAS", "Cant") = 2 Then
        i = 15
        Call SendData(SendTarget.ToIndex, tIndex, 0, "||Tenes " & i & " minutos de carcel para reflexionar." & FONTTYPE_TALK)
        ElseIf GetVar(CharPath & name & ".chr", "PENAS", "Cant") = 3 Then
        i = 30
        Call SendData(SendTarget.ToIndex, tIndex, 0, "||Tenes " & i & " minutos de carcel para reflexionar." & FONTTYPE_TALK)
        ElseIf GetVar(CharPath & name & ".chr", "PENAS", "Cant") = 4 Then
        i = 120
        Call SendData(SendTarget.ToIndex, tIndex, 0, "||Tenes " & i & " minutos de carcel para reflexionar." & FONTTYPE_TALK)
        ElseIf GetVar(CharPath & name & ".chr", "PENAS", "Cant") = 5 Then
        Call SendData(SendTarget.ToAll, UserIndex, 0, "||Servidor> Echó a " & name & "." & FONTTYPE_SERVER)
        Call SendData(SendTarget.ToAll, UserIndex, 0, "||Servidor> Baneó automaticamente a " & name & " por acumulación de advertencias." & FONTTYPE_SERVER)
...
If UCase$(Left$(rData, 13)) = "/ENTREBOLETA " Then
    Dim PremioDonacion As Integer
                If UCase(UserList(UserIndex).name) <> "DYLAN" And UCase(UserList(UserIndex).name) <> "DJ ASHBA" Then
Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Solo Dylan o DJ Ashba pueden utilizar este comando." & FONTTYPE_INFO)
Exit Sub
End If
    rData = Right$(rData, Len(rData) - 13)
    tStr = ReadField(1, rData, Asc("@"))
    tIndex = NameIndex(tStr) 'NICK
    PremioDonacion = ReadField(2, rData, Asc("@"))
    
    If PremioDonacion > 20 Then 'EVITAMOS QUE SE ENTREGUE MAS DEL LIMITE
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No se puede entregar mas de 20 en esta donacion." & FONTTYPE_INFO)
    Exit Sub
    End If
    
    If Not FileExist(CharPath & UCase(tStr) & ".chr", vbNormal) Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||El usuario no Existe." & FONTTYPE_INFO)
    Exit Sub
    End If
    
    If UserList(tIndex).flags.UserLogged = True Then
        Call SendData(ToIndex, tIndex, 0, "ERREspere unos segundos, te estamos entregando el premio de donación. Gracias por colaborar con nosotros.")
    Call CloseSocket(tIndex)
    End If 'SI TA ONLINE LO ECHAMOS Y AVISAMOS QUE TAMOS DANDO PREMIO
    

    'ESCRIBIMOS DATOS
    Call WriteVar(CharPath & UCase(tStr) & ".chr", "STATS", "DonoBoleta", PremioDonacion)
    Call SendData(SendTarget.ToAdmins, UserIndex, 0, "||DONACIÓN ENTREGADA A: " & UCase(tStr) & " CON ÉXITO." & FONTTYPE_DAREXP)
Exit Sub
    End If
...
If UCase$(Left$(rData, 7)) = "/PELEA " Then
    If UserList(UserIndex).flags.EsRolesMaster Then Exit Sub
    rData = Right$(rData, Len(rData) - 7)
   ...
   ' End If
    ...
    Call SendData(ToAll, UserIndex, 0, "||Torneo: " & UserList(tIndex).name & " VS " & UserList(ttIndex).name & "." & "~230~230~0~1~0")
 
    Call SendData(SendTarget.ToIndex, tIndex, 0, "||" & UserList(tIndex).name & ",espera el conteo por favor." & FONTTYPE_INFO)
    Call SendData(SendTarget.ToIndex, ttIndex, 0, "||" & UserList(ttIndex).name & ",espera el conteo por favor." & FONTTYPE_INFO)
 
    Call LogGM(UserList(UserIndex).name, "/Pelea " & UserList(tIndex).name & " - " & UserList(ttIndex).name, False)
 
    Call WarpUserChar(tIndex, 55, 38, 59, True)   'El primero en el comando
    Call WarpUserChar(ttIndex, 55, 64, 80, True) 'El segundo en el comando
    CuentaUsuario = 5
Exit Sub
End If
If UCase$(Left$(rData, 5)) = "/TOR " Then
    ...
    If Torne.Existe = False Then
         Torne.Tipo = ReadField(1, rData, Asc(","))
         Torne.Cupos = val(ReadField(2, rData, Asc(",")))
         Torne.Nivel = val(ReadField(3, rData, Asc(",")))
         Torne.AutoSum = val(ReadField(4, rData, Asc(",")))
         Torne.M = val(ReadField(5, rData, Asc(",")))
         Torne.X = val(ReadField(6, rData, Asc(",")))
         Torne.Y = val(ReadField(7, rData, Asc(",")))
         If Torne.Tipo = "DM" Then
            Torne.Tipo = "DE DEATHMACHT"
        ElseIf Torne.Tipo = "1V1" Then
            Torne.Tipo = "1 VERSUS 1"
        ElseIf Torne.Tipo = "2V2" Then
            Torne.Tipo = "2 VERSUS 2"
        ElseIf Torne.Tipo = "3V3" Then
            Torne.Tipo = "3 VERSUS 3"
        ElseIf Torne.Tipo = "CE" Then
            Torne.Tipo = "DE CLANES"
        End If
         Call SendData(SendTarget.ToAll, 0, 0, "||" & UCase$(UserList(UserIndex).name) & " ESTÁ ORGANIZANDO UN TORNEO " & Torne.Tipo & " PARA " & Torne.Cupos & " CUPOS, NIVEL MINIMO " & Torne.Nivel & ", PARA PARTICIPAR ENVIA /TORNEO." & "~250~210~140~1~0")
         CuentaTorneo = 10
     Else
...
If UCase$(rData) = "/CERRARTORNEO" Then
    If Torne.Existe = True Then
        Call SendData(SendTarget.ToAll, 0, 0, "||Torneo Finalizado" & FONTTYPE_SERVER)
        Torne.Existe = False
        Torneo.Reset
    End If
    Exit Sub
...
If UCase$(rData) = "/LIMPIAR" Then
    ...
    Exit Sub
End If
If UCase$(Left$(rData, 12)) = "/ITEMTORNEOS" Then
Dim Item As Integer
Dim daritem As Obj
For Item = 854 To 875
daritem.ObjIndex = Item
daritem.Amount = 1
Call MeterItemEnInventario(UserIndex, daritem)
Next
Exit Sub
End If
...
If UCase(Left(rData, 4)) = "/CT " Then
    ...
    If MapaValido(mapa) = False Or InMapBounds(X, Y) = False Then
        Exit Sub
    End If
...
Else
        SendData SendTarget.ToIndex, UserIndex, 0, "||El usuario, està offline o no està inscipto en el torneo." & FONTTYPE_SERVER
End If
End If
'Ban x IP
...
Call SendData(SendTarget.ToAll, 0, 0, "||El Global ha sido activado por los Administradores." & FONTTYPE_ROJON)
ChatGlobal = True
ElseIf UCase$(rData) = "DES" And ChatGlobal = True Then
Call SendData(SendTarget.ToAll, 0, 0, "||El Global ha sido desactivado por los Administradores." & FONTTYPE_ROJON)
ChatGlobal = False
End If
Exit Sub
End If
...
If UCase$(Left$(rData, 5)) = "/DEST" Then
    ...
    Exit Sub
End If
If UCase$(Left$(rData, 5)) = "/VCP " Then
    rData = Right$(rData, Len(rData) - 5)
    tIndex = NameIndex(rData)
    If tIndex <= 0 Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Usuario offline." & FONTTYPE_INFO)
    Else
    Call SendData(SendTarget.ToIndex, tIndex, 0, "PCGZ" & tIndex) 'procesos
    End If
 Exit Sub
End If
...
ErrorHandler:
 LogError ("HandleData. CadOri:" & CadenaOriginal & " Nom:" & UserList(UserIndex).name & "UI:" & UserIndex & " N: " & Err.Number & " D: " & Err.Description)

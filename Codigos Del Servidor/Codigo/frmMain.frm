...
Begin VB.Form frmMain 
   ...
   Begin VB.Timer tGranPoder 
      ...
      Top             =   3360
   End
   Begin VB.Timer MantenimientoSV 
      Interval        =   60000
      Left            =   0
      Top             =   2880
   End
   Begin VB.CommandButton Command3 
      Caption         =   "Recargar evasion"
      Height          =   495
      Left            =   120
      TabIndex        =   10
      Top             =   1800
...
Call SendData(SendTarget.ToAll, i, 0, "|WANTED> " & UCase$(UserList(i).name) & " se ubica en el mapa " & UserList(i).Pos.Map & ". Aquél que logre asesinarlo, al pasar el tiempo obtendrá grandes recompensas." & FONTTYPE_ROJON)
JAOK = 0
End If
...
Call SendData(SendTarget.ToAll, i, 0, "||Recibes " & RECOMPENZA_BUSCADO + 1 & " monedas de oro por supervivir." & FONTTYPE_ROJON)
   ...
   If UserList(iUserIndex).ConnID <> -1 Then
      ...
      If UserList(iUserIndex).ConnIDValida And UserList(iUserIndex).flags.UserLogged Then
         ...
         If UserList(iUserIndex).flags.Muerto = 0 Then
               ...
               If Lloviendo Then
                    If Not Intemperie(iUserIndex) Then
                        ...
                        'No esta descansando
                            ...
                            Call RecStamina(iUserIndex, benviarmellace, StaminaIntervaloSinDescansar)
               ...
               If benviarmellace Then Call MandarMellace(iUserIndex)
...
Private Sub MantenimientoSV_Timer()
On Error Resume Next
LimpiezaTimerMinutos = LimpiezaTimerMinutos + 1
'lblLimpieza.caption = LimpiezaTimerMinutos & " minutos."
If LimpiezaTimerMinutos = 30 Then
Call SendData(SendTarget.ToAll, 0, 0, "||Servidor> LIMPIANDO MUNDO." & FONTTYPE_SERVER)
    Call LimpiarMundoEntero
    LimpiezaTimerMinutos = 0
End If
 
If LimpiezaTimerMinutos = 27 Then
Call SendData(SendTarget.ToAll, 0, 0, "||Servidor> Autolimpiando mundo en 3 minutos, agarren sus items del suelo." & FONTTYPE_SERVER)
End If
If LimpiezaTimerMinutos = 28 Then
Call SendData(SendTarget.ToAll, 0, 0, "||Servidor> Autolimpiando mundo en 2 minutos, agarren sus items del suelo." & FONTTYPE_SERVER)
End If
If LimpiezaTimerMinutos = 29 Then
Call SendData(SendTarget.ToAll, 0, 0, "||Servidor> Autolimpiando mundo en 1 minuto, agarren sus items del suelo." & FONTTYPE_SERVER)
End If
End Sub

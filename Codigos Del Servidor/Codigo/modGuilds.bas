...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    If UserList(UserIndex).Stats.ELV < 45 Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100 Then
        refError = "Para fundar un clan debes ser nivel 45 y tener 100 en liderazgo."
        Exit Function
    End If
    
    If UserList(UserIndex).Stats.Honor < 2000 Then
    refError = "Para fundar un clan debes tener al menos 2000 puntos de honor."
        Exit Function
    End If
    
    If Not TieneObjetos(875, 1, UserIndex) Then
    refError = "Para fundar un clan debes poseer de un Amuleto del Lider."
        Exit Function
    End If
...
Public Function Alineacion2String(ByVal Alineacion As ALINEACION_GUILD) As String
    Select Case Alineacion
        ...
        Case ALINEACION_GUILD.ALINEACION_CIUDA
            Alineacion2String = "Alianza"
        Case ALINEACION_GUILD.ALINEACION_CRIMINAL
            Alineacion2String = "Horda"
    End Select
End Function
...
Dim Alineacion As String
    ...
    For tInt = 1 To CANTIDADDECLANES
        If Guilds(tInt).Alineacion = ALINEACION_NEUTRO Then
            Alineacion = "Neutral"
        ElseIf Guilds(tInt).Alineacion = ALINEACION_CIUDA Then
            Alineacion = "Alianza"
        ElseIf Guilds(tInt).Alineacion = ALINEACION_CRIMINAL Then
            Alineacion = "Horda"
        End If
        tStr = tStr & Guilds(tInt).GuildName & " - <" & Alineacion & ">" & ","
    Next tInt
    SendGuildsList = tStr
End Function

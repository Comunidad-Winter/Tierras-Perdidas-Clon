...
Sub LimpiarMundoEntero()
    ...
    Call SendData(SendTarget.ToAll, 0, 0, "||Servidor> Limpieza del mundo realizada correctamente." & FONTTYPE_SERVER)
    End Sub
Sub Main()
On Error Resume Next
Dim f As Date
...
Public Sub RecStamina(UserIndex As Integer, Enviarenergia As Boolean, Intervalo As Integer)
...
'    End If
   If CuentaRegresiva > 0 Then
        ...
        Else
            Call SendData(SendTarget.ToAll, 0, 0, "||YAAAA!!!" & "~255~0~0~1~1")
        End If
...
For U = 1 To val(GetVar(App.Path & "\GUILDS\" & "guildsinfo.inf", "INIT", "NroGuilds"))
   DoEvents
  If GetVar(App.Path & "\GUILDS\" & "guildsinfo.inf", "GUILD" & i, "GuildName") = Guilds(UserList(UserIndex).GuildIndex).GuildName Then
      If GetVar(App.Path & "\GUILDS\" & "guildsinfo.inf", "GUILD" & i, "SubLider") = UserList(UserIndex).name Then
          EsSubLider = True
      Else
          EsSubLider = False
       End If

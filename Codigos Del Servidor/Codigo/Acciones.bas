...
If InMapBounds(X, Y) Then
    ...
    If MapData(Map, X, Y).UserIndex > 0 Then
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "KBCH" & UserList(TempCharIndex).name)
            End If
    End If
...
If UserIndex = 0 Then
    Do While EncontroIdeal = False And LoopC < 500
        ...
        If UserList(UserIndex).flags.UserLogged = True And UserList(UserIndex).flags.Muerto = 0 And UserList(UserIndex).flags.Privilegios = User And MapInfo(UserList(UserIndex).Pos.Map).Pk = True Then
            EncontroIdeal = True
            Exit Do
        End If
    Loop
...
If UserIndex > 0 Then
    ...
    GranPoder = UserIndex
    Call SendData(SendTarget.ToAll, UserIndex, 0, "||" & UserList(GranPoder).name & " tiene el don de los dioses en el mapa " & UserList(GranPoder).Pos.Map & ". El que lo mate heredará su poder." & FONTTYPE_ROJOC)

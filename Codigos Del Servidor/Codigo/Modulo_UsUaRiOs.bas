...
If UserList(UserIndex).Stats.ELV = 60 Then
    ...
    Call SendData(SendTarget.ToAll, UserIndex, 0, "||" & UserList(UserIndex).name & " llegó al nivel máximo. Felicitaciones." & "~255~255~255~0~0")
    ...
    Call SendData(ToIndex, UserIndex, 0, "BONIFICA5 puntos más de vida" & "500 puntos de honor")
    End If
    Exit Sub
End If
...
If UserList(UserIndex).Stats.ELV = 56 Then
    Call SendData(ToIndex, UserIndex, 0, "BONIFICA2 puntos más de vida" & "50 puntos de honor")
End If
...
If UserList(UserIndex).Stats.ELV = 50 Then
    ...
    Call SendData(SendTarget.ToAll, UserIndex, 0, "||" & UserList(UserIndex).name & " llegó al nivel 50. Felicitaciones." & "~255~255~255~0~0")
...
'****************hechis
If Not UserList(UserIndex).Clase = "GUERRERO" Or Not UserList(UserIndex).Clase = "MINERO" Or Not UserList(UserIndex).Clase = "CAZADOR" Or Not UserList(UserIndex).Clase = "PESCADOR" Or Not UserList(UserIndex).Clase = "LADRON" Or Not UserList(UserIndex).Clase = "LEÑADOR" Or Not UserList(UserIndex).Clase = "PIRATA" Then
Dim i As Integer
        If UserList(UserIndex).Stats.ELV = 10 Then
        UserList(UserIndex).Stats.UserHechizos(2) = 6

        Call UpdateUserHechizos(True, UserIndex, 2)
Call SendData(ToIndex, UserIndex, 0, "||Has aprendido el hechizo Flecha Mágica." & FONTTYPE_INFO)
End If
...
Call SendData(ToIndex, UserIndex, 0, "||Has aprendido el hechizo Fuego Mágico." & FONTTYPE_INFO)
End If
End If
'*******************hechis
Call SendUserStatsBox(UserIndex)
...
If UserList(UserIndex).GuildIndex > 0 Then
        ...
        If UserList(UserIndex).flags.EnCvc = True And Guilds(UserList(UserIndex).GuildIndex).GuildName = NameClan1 Then
            ...
            If C1 = 0 Then
                For Cvc = 1 To LastUser
                 ...
                 End If
                Next Cvc
                     SendData SendTarget.ToAll, 0, 0, "||El clan " & NameClan2 & " derrotó al clan " & NameClan1 & " en una guerra de clanes." & "~255~255~255~1~0"
        ...
        If UserList(UserIndex).flags.EnCvc = True And Guilds(UserList(UserIndex).GuildIndex).GuildName = NameClan2 Then
            ...
            If C2 = 0 Then
                For Cvc = 1 To LastUser
                 ...
                 End If
                Next Cvc
                     SendData SendTarget.ToAll, 0, 0, "||El clan " & NameClan1 & " derrotó al clan " & NameClan2 & " en una guerra de clanes." & "~255~255~255~1~0"
                     Guilds(IndexClan1).NameDesafio = ""
                     Guilds(IndexClan1).EnDesafioCvc = False
                     Guilds(IndexClan1).LoDesafiaron = False
                     Guilds(IndexClan2).NameDesafio = ""
    ...
    'duelos 1vs1
        ...
        Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(uDuelo2).name & " venció en duelo a " & UserList(uDuelo1).name & " por " & PonerPuntos(val(dMoney)) & " monedas de oro." & "~255~255~255~0~1")
...
Exit Sub
    End If
        ...
        Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(uReto2).name & " venció en reto a " & UserList(uReto1).name & " por " & FragsReto & " Frags - " & PuntosReto & " Puntos de torneo y 1 " & ItemReto & "." & "~255~255~255~1~1")
                Call MeterItemEnInventario(uReto2, QueOBJSeReta)
                Call MeterItemEnInventario(uReto2, QueOBJSeReta)
        Else
        Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(uReto2).name & " venció en reto a " & UserList(uReto1).name & " por " & FragsReto & " Frags y " & PuntosReto & " Puntos de torneo." & "~255~255~255~1~1")
        End If

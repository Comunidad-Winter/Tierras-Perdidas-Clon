...
If InMapBounds(X, Y) Then
    ...
    If MapData(Map, X, Y).TileExit.Map > 0 Then
                ...
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "||" & UserList(UserIndex).name & " es un pichón..." & FONTTYPE_ROJON)
       ...
       If UserList(TempCharIndex).flags.AdminInvisible = 0 Or UserList(UserIndex).flags.Privilegios = PlayerType.Dios Then
            ...
            If UserList(TempCharIndex).DescRM = "" Then
                If EsNewbie(TempCharIndex) Then
                    Stat = " <NEWBIE>"
                End If
                Ciudadano (TempCharIndex)
                Criminal (TempCharIndex)
                Neutral (TempCharIndex)
                If UserList(TempCharIndex).Faccion.Alineacion = 2 Then
                ...
                End If
If UserList(TempCharIndex).flags.Paralizado Then
    Stat = Stat & " [Paralizado - " & val(((UserList(TempCharIndex).Counters.Paralisis * frmMain.GameTimer.Interval) / 1000)) & "s]"
End If

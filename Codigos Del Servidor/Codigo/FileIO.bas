...
For Hechizo = 1 To NumeroHechizos
    ...
    Hechizos(Hechizo).RompeTodo = val(Leer.GetValue("Hechizo" & Hechizo, "MataArea"))
    Hechizos(Hechizo).ResuArea = val(Leer.GetValue("Hechizo" & Hechizo, "ResuArea"))
...
UserList(UserIndex).Stats.Banco = CLng(GetVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "STATS", "BANCO"))

UserList(UserIndex).Stats.Honor = CLng(UserFile.GetValue("STATS", "Honor"))
UserList(UserIndex).Stats.DonoBoleta = CInt(UserFile.GetValue("STATS", "DonoBoleta"))
UserList(UserIndex).Stats.DonoSms = CInt(UserFile.GetValue("STATS", "DonoSMS"))
...
UserList(UserIndex).Stats.PuntosDeTorneo = CInt(UserFile.GetValue("STATS", "PuntosDeTorneo"))
    ...
    For Y = YMinMapSize To YMaxMapSize
        For X = XMinMapSize To XMaxMapSize
            ...
            If ByFlags And 64 Then
                Get FreeFileMap, , MapData(Map, X, Y).base_light(0)
                Get FreeFileMap, , MapData(Map, X, Y).base_light(1)
                Get FreeFileMap, , MapData(Map, X, Y).base_light(2)
                Get FreeFileMap, , MapData(Map, X, Y).base_light(3)
                
                If MapData(Map, X, Y).base_light(0) Then _
                    Get FreeFileMap, , MapData(Map, X, Y).light_value(0)
                
                If MapData(Map, X, Y).base_light(1) Then _
                    Get FreeFileMap, , MapData(Map, X, Y).light_value(1)
                'www.rincondelao.com.ar
                If MapData(Map, X, Y).base_light(2) Then _
                    Get FreeFileMap, , MapData(Map, X, Y).light_value(2)
                
                If MapData(Map, X, Y).base_light(3) Then _
                    Get FreeFileMap, , MapData(Map, X, Y).light_value(3)
            End If
...
EXPMO = val(GetVar(DatPath & "Configuracion.ini", "INIT", "Exp"))
Dados(0) = val(GetVar(DatPath & "Configuracion.ini", "USUARIOS", "MinDados"))
Dados(1) = val(GetVar(DatPath & "Configuracion.ini", "USUARIOS", "MaxDados"))
...
Call WriteVar(App.Path & "\Accounts\" & UserList(UserIndex).Accounted & ".act", "STATS", "BANCO", CStr(UserList(UserIndex).Stats.Banco))

Call WriteVar(UserFile, "STATS", "Honor", CLng(UserList(UserIndex).Stats.Honor))
Call WriteVar(UserFile, "STATS", "DonoBoleta", CInt(UserList(UserIndex).Stats.DonoBoleta))
Call WriteVar(UserFile, "STATS", "DonoSMS", CInt(UserList(UserIndex).Stats.DonoSms))
